...
export class AiRepository {
  ...
  // Returns a stream of text chunks from the AI provider (real or mock)
  public getStream(messages: ChatMessage[]): AsyncIterable<string> {
    const useMock = process.env.USE_MOCK === 'true';

    if (useMock) {
      console.info('⚠️ [AiRepository] Using MOCK mode');
      return this.getMockStream();
    }

    if (!this.apiKey) {
      console.error('❌ [AiRepository] No API Key provided');
  ...
  // A real request to the Groq API with streaming response
